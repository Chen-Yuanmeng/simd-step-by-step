            ...
            for (int jb = 0; jb < N; jb += BLOCK)
                for (int i = ib; i < ib + BLOCK; i++)
                    for (int k = kb; k < kb + BLOCK; k++) {
                        __m512i aik = _mm512_set1_epi32(A[i * N + k]);
                        for (int j = jb; j < jb + BLOCK; j += VLEN) {
                            __m512i c = _mm512_loadu_si512((__m512i *)(C + i * N + j));
                            __m512i b = _mm512_loadu_si512((__m512i *)(B + k * N + j));
                            __m512i mul = _mm512_mullo_epi32(b, aik);
                            c = _mm512_add_epi32(c, mul);
                            _mm512_storeu_si512((__m512i *)(C + i * N + j), c);
                        }
                    }
